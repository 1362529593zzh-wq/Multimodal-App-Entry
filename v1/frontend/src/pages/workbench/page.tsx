...
const parseWorkbenchResult = (contentText: string | null, relatedTaskId: string | null): WorkbenchResultData => {
  ...
  try {
    ...
    return {
      ...
      pptDraft: parsed.pptDraft,
      deckPlan: parsed.deckPlan,
      previewSlides: parsed.previewSlides,
...
export const WorkbenchPage = () => {
  ...
  const generatePptFileMutation = useMutation({
    mutationFn: ({ taskId, draft }: { taskId: string; draft: PptDeckDraft }) => generatePptFile(taskId, draft),
  });
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const submitOptions =
        currentCapability.functionCode === 'text_to_ppt'
          ? { pptMode: 'ai_full', ...currentDraftOptions }
          : currentDraftOptions;
      ...
      const envelope = await submitChatMutation.mutateAsync({
        ...
        inputText: prompt,
        serviceCode: submitOptions.model,
        model: submitOptions.model,
        options: JSON.stringify(submitOptions),
        composerOptions: JSON.stringify(submitOptions),
        schemaVersion: 'composer_schema_v1',
        templateCode: submitOptions.template,
        attachments: submitOptions.referenceImageFileId
          ? JSON.stringify([{ fileId: submitOptions.referenceImageFileId, role: 'referenceImage' }])
          : undefined,
        inheritanceMode: followUp ? 'same_capability' : 'none',
        parentTaskId: followUp?.taskId,
        sourceAssetIds: submitOptions.referenceImageFileId
          ? JSON.stringify([submitOptions.referenceImageFileId])
          : '[]',
      });
  ...
  const handleGeneratePpt = async (messageItem: WorkbenchMessageItem, draft: PptDeckDraft) => {
    const taskId = messageItem.result?.chips.find((chip) => chip.startsWith('task: '))?.replace('task: ', '');
    if (!taskId) {
      void message.warning('未找到原始 PPT 任务，无法生成文件。');
      return;
    }

    try {
      const result = await generatePptFileMutation.mutateAsync({ taskId, draft });
      Modal.success({
        title: 'PPT 已生成',
        content: result.fileName ? `文件：${result.fileName}` : '已根据编辑后的大纲生成 PPT 文件。',
        okText: '知道了',
      });
      if (result.downloadUrl) {
        window.open(result.downloadUrl, '_blank');
      }
      await queryClient.invalidateQueries({ queryKey: ['workbench', 'messages', resolvedConversationId] });
    } catch {
      // http.ts 已统一提示错误，这里不重复弹出。
    }
  };
    ...
    <div className="workbench-page">
      <WorkbenchShell sidebar={sidebar}>
        <MessageStream
          ...
          generatingPptTaskId={generatePptFileMutation.isPending ? generatePptFileMutation.variables?.taskId : undefined}
          onGeneratePpt={(messageItem, draft) => {
            void handleGeneratePpt(messageItem, draft);
          }}
