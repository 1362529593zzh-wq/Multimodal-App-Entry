...
const pptGenerationDefaults: WorkbenchFieldDefinition[] = [
  {
    key: 'pptMode',
    label: 'PPT 模式',
    placeholder: '选择生成模式',
    options: [
      { label: 'AI 智能设计', value: 'ai_full' },
      { label: '快速大纲', value: 'legacy' },
    ],
  },
  {
    key: 'pages',
    label: '页数',
    placeholder: '选择页数',
    options: ['6', '8', '10', '12'].map((value) => ({ label: value, value })),
  },
];
  ...
  const fields =
    capabilityCode === 'image_generation'
      ...
      : capabilityCode === 'text_to_video'
        ...
        : capabilityCode === 'text_to_ppt'
          ? mergeFields(blueprint.fields, pptGenerationDefaults)
          : blueprint.fields;
        ...
        return (
          ...
          >
            <Button
              ...
              title={selectedValue ? `${label}\uff1a${selectedOptionLabel ?? selectedValue}` : label}
            >
              {buttonLabel}
              <DownOutlined />
            </Button>
